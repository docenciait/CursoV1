...
2.  **Validadores Personalizados (`Custom Validators` - Estilo Pydantic V2):**
    Para lógica de validación que va más allá de los `constraints` estándar.
      ...
      * **`@model_validator(mode='before'|'after')`:**
        ...
        *(En Pydantic V2, el acceso a otros campos dentro de un `@field_validator` para validaciones cruzadas es menos directo; `@model_validator` es la forma preferida para validaciones inter-campos.)*
...
**Beneficios Concretos de Pydantic y los `Nested Models` en FastAPI 🏆**
  ...
  * **Claridad y Robustez del Contrato de API:** Tus modelos Pydantic *son* la definición formal de los datos que tu API espera y devuelve. Sirven como una `source of truth`.
...
**Consejos Prácticos Adicionales:**
  ...
  * **Deshabilitar en Producción (Opcional):** Si por razones de seguridad no quieres exponer `/docs`, `/redoc`, o `/openapi.json` en tu entorno de producción, puedes hacerlo al instanciar `FastAPI`:
...
Vamos a construir un ejemplo:
  ...
  * **Diagrama `Mermaid` del Flujo de Autenticación `JWT`:**
    ...
    sequenceDiagram
        ...
        participant S_Auth as Auth Server (FastAPI /token)
        participant S_Resource as Resource Server (FastAPI /users/me)

        C->>+S_Auth: POST /auth/token (username, password)
        S_Auth->>S_Auth: Validate Credentials
        S_Auth->>S_Auth: Generate JWT (access_token)
        S_Auth-->>-C: Response: {access_token: "xxx.yyy.zzz", token_type: "bearer"}
        
        Note over C: Client stores access_token
        
        C->>+S_Resource: GET /users/me <br/>Header: Authorization: Bearer xxx.yyy.zzz
        S_Resource->>S_Resource: Depends(get_current_active_user)
        Note over S_Resource: OAuth2PasswordBearer extracts token
        Note over S_Resource: JWT decoded & validated (signature, exp, claims)
        Note over S_Resource: User identified from token
        S_Resource-->>-C: Response: User Data JSON
    ```
...
Nuestro `ConnectionManager` de 10.3 era un buen comienzo. Podemos refinarlo:
  ...
  * **Limpieza Aún Más Robusta en `disconnect`:**
    Asegurar que cuando un `WebSocket` se desconecta, se elimine de *todas* las estructuras donde esté referenciado (lista general, todas las `rooms`, mapeos `user_id` a `connection_id`). El `ConnectionManager` de 10.3 (versión refinada) ya intentaba esto.
...
Cuando un `handler` `WebSocket` recibe datos (`on_receive_message` en nuestro ejemplo de clase), a menudo necesita:
  ...
  * **Invocando Lógica de Negocio (`Application Services` / `Use Cases`):**
    **Crucial:** El `WebSocket handler` es un **Adaptador** en una Arquitectura Hexagonal. Su trabajo es traducir el `message` `WebSocket` en una llamada a un `service` de aplicación o `use case` en tu `core business logic`. **No debe contener lógica de negocio compleja directamente.**
...
Cuando un `client` envía un `message` a través de un `WebSocket` que implica una acción de negocio (ej. enviar un mensaje de chat, realizar una puja, mover una pieza en un juego).
  ...
  * **Respuesta y Notificaciones Subsecuentes:**
      ...
      * **`Acknowledgement` (Ack) Directo (Opcional):** El `Application Service` podría devolver un resultado simple (ej. un ID de confirmación, un `status` de éxito/error) al `WebSocket Handler`. Este, a su vez, podría enviar una respuesta directa *solo al `client` WebSocket original* para confirmar que su `message` fue recibido y procesado inicialmente.
        ```python
        ...
        ```
      * **`Updates` a Otros `Clients` (Impulsados por `Domain Events`):** Los `Domain Events` generados son la forma preferida de notificar a *otros* `clients` (y potencialmente al `client` original también, de forma consistente) sobre el cambio de estado. Esto se maneja en el Flujo 2.
  ...
  * **Diagrama de Secuencia (Simplificado):**
    ...
    sequenceDiagram
        ...
        participant WSHandler as FastAPI WS Handler (Adapter)
        participant AppService as Application Service (Port)
        participant DomainAgg as Domain Aggregate
        participant EventBus as Domain Event Bus / Dispatcher

        ClientWS->>+WSHandler: Envía Mensaje WS (ej. nuevo chat)
        WSHandler->>WSHandler: Valida/Transforma Mensaje
        WSHandler->>+AppService: Llama a `submit_chat_message(data)`
        AppService->>+DomainAgg: Carga/Crea Aggregate y ejecuta método
        DomainAgg->>DomainAgg: Aplica lógica de negocio
        DomainAgg-->>-EventBus: Genera `ChatMessageSentEvent`
        AppService-->>-WSHandler: (Opcional) Devuelve Ack/Resultado Inmediato
        WSHandler-->>-ClientWS: (Opcional) Envía Ack/Resultado Inmediato
    ```
...
Este es el flujo para `server-initiated pushes` donde un cambio en el `backend` (iniciado por una petición HTTP, otro `message` `WebSocket`, un `background job`, etc.) necesita ser comunicado a los `WebSocket clients` relevantes.
  ...
  * **`Event Handlers` (Escuchando los Ecos del Dominio):**
      ...
      * Cuando un `Domain Event` es despachado (ej. por una `Unit of Work` al hacer `commit`, o por un `event bus` `in-process` o externo si los `event handlers` son `microservices` separados):
        ...
        4.  **Publicación en el `WebSocket Pub/Sub Backend`:** El `Event Handler` **publica** el `message` `WebSocket` formateado en el `channel` o `topic` apropiado de nuestro `Pub/Sub backend` (Redis/Kafka, como vimos en 10.4 y 10.6). *No interactúa directamente con las `WebSocket connections` individuales si operamos en un entorno distribuido/escalado.*
  ...
  * **Las `FastAPI WebSocket Gateway Instances` Hacen el Trabajo Final:**
    ...
    3.  Usan su `ConnectionManager` local (de 10.3/10.10) para encontrar los `WebSocket clients` actualmente conectados a *esa `instance`* que pertenecen a la `room` o son el `target user`.
  ...
  * **Diagrama de Secuencia (Simplificado):**
    ...
    ```mermaid
    sequenceDiagram
        participant BizLogic as Backend Business Logic / Domain
        participant DomainEvBus as Domain Event Bus / Dispatcher
        participant WSEventHandler as WebSocket Domain Event Handler
        participant WSPubSub as WebSocket Pub/Sub Backend (Redis/Kafka)
        participant FastAPI_WS_GW as FastAPI WS Gateway Instance(s)
        participant ClientWS as Relevant WebSocket Client(s)

        BizLogic->>BizLogic: Ocurre cambio de estado
        BizLogic-->>DomainEvBus: Publica `DomainEvent` (ej. `OrderShippedEvent`)
        DomainEvBus-->>WSEventHandler: Despacha `DomainEvent`
        WSEventHandler->>WSEventHandler: Transforma `DomainEvent` a `WS_Message_Payload`
        WSEventHandler-->>WSPubSub: Publica `WS_Message_Payload` a `channel/topic` (ej. `user_notifications:U456`)
        WSPubSub-->>FastAPI_WS_GW: `WS_Message_Payload` llega a la(s) instancia(s)
        FastAPI_WS_GW->>FastAPI_WS_GW: Usa `ConnectionManager` local para encontrar `clients`
        FastAPI_WS_GW-->>ClientWS: Envía `WS_Message_Payload`
    ```
