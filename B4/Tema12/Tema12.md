...
**2. Escalado Horizontal (`Scaling Out`): Multiplicando la Fuerza Laboral 🏙️➡️🏙️🏙️**
  ...
  * **Diagrama `Mermaid`:**
    ...
    subgraph Sistema_ANTES_del_Escalado_Horizontal
...
5.  **Periodo de `Cooldown`:** Después de una acción de escalado, suele haber un periodo de espera (`cooldown`) antes de que se pueda realizar otra acción, para evitar oscilaciones rápidas ("flapping").
  ...
  * **Diagrama `Mermaid` del Ciclo del HPA:**
    ```mermaid
      graph TD
    A[1 · Definir HPA - target CPU 70% · min 2 · max 10] --> B[2 · HPA Controller]
    B -- Consulta periódica --> MS[Metrics Server · uso CPU/Mem]
    MS -- Métricas actuales --> B
    B -- 3 · Calcular replicas deseadas · ej: CPU 85% --> B
    B -- 4 · Actualizar replicas en Deployment --> D[Kubernetes Deployment]
    D --> P1[Pod 1]
    D --> P2[Pod 2]
    D --> P3[Pod 3 ...]
...
Ambos tipos de `affinity`/`anti-affinity` pueden tener dos modos:
  ...
  * **`requiredDuringSchedulingIgnoredDuringExecution`**: Regla **estricta**. El `Pod` *no será programado* a menos que la regla se cumpla. Si las condiciones del `Node` cambian *después* de que el `Pod` está corriendo (ej. un `label` del `Node` se elimina), el `Pod` seguirá corriendo allí.
  * **`preferredDuringSchedulingIgnoredDuringExecution`**: Regla **suave** (preferencia). El `scheduler` *intentará* cumplir la regla, pero si no es posible, programará el `Pod` de todas formas donde pueda. A cada preferencia se le asigna un `weight`.
...
*El `Scheduler` de Kubernetes intentará colocar cada `Pod` con el `label` `app: my-fastapi-app` en un `Node` con un `hostname` diferente.*
...
Los **`taints`** y **`tolerations`** trabajan juntos para asegurar que los `Pods` no sean programados en `Nodes` inapropiados.
  ...
  * **`Tolerations` (Tolerancias):**
      ...
      * Se aplican a los **`Pods`**. Una `toleration` permite que un `Pod` *sea programado* en un `Node` que tiene un `taint` coincidente. No garantiza que el `Pod` vaya a ese `Node`, solo que *puede* ir.

  * **Efectos de los `Taints`:**

      * **`NoSchedule`**: Los `Pods` sin una `toleration` coincidente *no serán programados* en el `Node` con `taint`. Los `Pods` que ya están corriendo en el `Node` no se ven afectados.
      * **`PreferNoSchedule`**: Es una versión "suave" de `NoSchedule`. El `scheduler` *intentará* no colocar `Pods` sin `toleration` en el `Node`, pero lo hará si no hay otra opción.
...
*El `Pod_ML` puede ir al `Node_GPU` porque tiene la `toleration`. El `Pod_API` no puede, y será programado en `Node_General`.*
...
Estos mecanismos no son mutuamente excluyentes; a menudo se usan en conjunto para lograr políticas de emplazamiento de `Pods` muy específicas y robustas. Por ejemplo:
  ...
  * Luego, usar `node affinity` en los `Pods` de ese `workload` para *preferir* o *requerir* esos `Nodes` (además de tener la `toleration`).
...
### `Batching`: El Poder de Agrupar para la Eficiencia  собирать группы
  ...
  * **Diagrama `Mermaid`: Flujo de `Batching`**
    ...
    ```mermaid
    graph TD
        I1[Ítem 1] --> BC[Batch Collector]
        I2[Ítem 2] --> BC
        I3[Ítem 3] --> BC
        IN[Ítem N] --> BC

        BC -- Llena Batch (Tamaño N o Timeout) --> PB{Procesar Batch}
        PB -- Operación Única --> SistemaExterno[DB / API / Queue]
        SistemaExterno -- Resultado del Batch --> PB
        PB -- Resultados individuales o agregado --> Salida

        style BC fill:#AED6F1,stroke:#3498DB
        style PB fill:#D5F5E3,stroke:#2ECC71
    ```
...
### `Debouncing`: Esperando el Momento Oportuno para Actuar ⏳
  ...
  * **Diagrama `Mermaid`: Flujo de `Debouncing`**
    ...
    ```mermaid
    sequenceDiagram
        participant UserAction as User/Event Source
        participant Debouncer
        participant TargetFunction as Función a Ejecutar

        UserAction->>Debouncer: Trigger 1 (t=0ms)
        Note over Debouncer: Inicia/Reinicia Timer (ej. 300ms)
        UserAction->>Debouncer: Trigger 2 (t=100ms)
        Note over Debouncer: Reinicia Timer (ej. 300ms)
        UserAction->>Debouncer: Trigger 3 (t=250ms)
        Note over Debouncer: Reinicia Timer (ej. 300ms)
        
        Note over Debouncer: Timer expira (t=550ms)<br/>(300ms después de Trigger 3)
        Debouncer->>TargetFunction: Ejecuta Función (con datos del Trigger 3)
    ```
  ...
  * **Implementación Conceptual de un `Debouncer` Asíncrono en Python:**
    ...
    *Este es un `debouncer` simplificado. Las implementaciones del mundo real pueden ser más complejas, a menudo proporcionadas por librerías de utilidades `async` o `frameworks` de `frontend`.*
...
Es fácil confundir `debouncing` con `throttling`, pero sirven a propósitos diferentes:
  ...
  * **`Debouncing` (Anti-rebote):** Ejecuta la función **solo una vez** después de que una ráfaga de `events` ha cesado por un periodo específico. Ideal cuando solo te interesa la *última* invocación de una serie. (Ej: Búsqueda "as-you-type" – solo buscas cuando el usuario para de teclear).
...
### 1\. Arquitectura de Software y Diseño General
  ...
  * **Cockburn, Alistair. *Hexagonal Architecture*. (Artículo online y discusiones).**
...
### 2\. `Microservices`
  ...
  * **Wolff, Eberhard. *Microservices: Flexible Software Architecture*. Addison-Wesley, 2016.** (También tiene material más reciente).
...
### 3\. CQRS y `Event Sourcing`
  ...
  * **Vernon, Vaughn. *Reactive Messaging Patterns with the Actor Model: Applications and Integration in Scala and Akka*. Addison-Wesley, 2015.**
      * Aunque enfocado en Actores, cubre muy bien los patrones reactivos, CQRS y `Event Sourcing`.
  * **Young, Greg. *CQRS and Event Sourcing*. (Artículo fundamental, busca "CQRS Documents by Greg Young").**
...
### 4\. Diseño de APIs RESTful
  ...
  * **Fielding, Roy T. *Architectural Styles and the Design of Network-based Software Architectures*. (Tesis doctoral, 2000).**
...
### 5\. FastAPI y Python `Async`
  ...
  * **Libros sobre `Asyncio` en Python:**
      * **Ramalho, Luciano. *Fluent Python: Clear, Concise, and Effective Programming*. O'Reilly Media, 2nd Edition, 2022.** (Tiene capítulos excelentes sobre `asyncio`).
      * **Serdar Yegulalp. *Python Concurrency with asyncio*. Manning, 2022.**
...
### 6\. Sistemas de Mensajería
  ...
  * **HSPC. *RabbitMQ in Depth*. Manning, 2018.** (Por Gavin M. Roy, el autor de Pika)
  ...
  * **Narkhede, Neha; Shapira, Gwen; & Palino, Todd. *Kafka: The Definitive Guide*. O'Reilly Media, 2nd Edition (si está disponible, o la 1ª Ed. 2017).**
...
### 8\. `Caching` y Redis
  ...
  * **Wilson, Andrew. *The Little Redis Book*. (Libro corto y gratuito, buen intro).**
...
### 10\. `Containerization` y `Orchestration` (Docker, Kubernetes)
  ...
  * **Hightower, Kelsey; Burns, Brendan; & Beda, Joe. *Kubernetes: Up and Running*. O'Reilly Media, 3rd Edition (o la más reciente).**
