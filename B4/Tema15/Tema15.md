...
**4. Helm: El Gestor de `Packages` para Kubernetes 📦**
  ...
  * **Diagrama Conceptual: `Helm Chart` a `Kubernetes Manifests`**
    ...
    CustomValues[values-prod.yaml - overrides para producción] -->|Proporcionado a| HelmCLI[Helm CLI - helm install o template]
...
**5. Kustomize: Personalización Declarativa de `YAML` sin `Templates` 🛠️**
  ...
  * **Diagrama Conceptual: Kustomize Base + `Overlay`**
    ...
    graph TD
        ...
        BaseDeployment --> KustomizationProd
...
Mientras que los `logs` te cuentan la historia de *lo que pasó*, las métricas te muestran *cómo está pasando* en tiempo real y de forma agregada.
...
**Flujo con Loki:**
`Microservice Logs -> Promtail -> Loki -> Grafana (con LogQL)`
...
Kubernetes, como orquestador, ofrece varias estrategias de `deployment` que facilitan los `rollouts` controlados:
  ...
  * **1. `Rolling Update` (El Estándar Progresivo de K8s):**
      ...
      * **Diagrama `Mermaid` de `Rolling Update`:**
        ...
        sequenceDiagram
            ...
            participant OldPods as Pods v1.0 (3 réplicas)
            participant NewPods as Pods v1.1 (0 réplicas)
            participant ServiceLB as K8s Service (Load Balancer)

            K8s->>OldPods: Inicialmente: 3 Pods v1.0 activos
            ServiceLB->>OldPods: Tráfico dirigido a v1.0
            
            Note over K8s: Inicia Rolling Update a v1.1
            K8s->>NewPods: Crea 1 Pod v1.1 (maxSurge)
            K8s->>NewPods: Espera que Pod v1.1 esté Ready
            K8s->>ServiceLB: Añade Pod v1.1 al pool del LB
            K8s->>OldPods: Termina 1 Pod v1.0 (maxUnavailable)
            ServiceLB->>OldPods: Tráfico ahora también a Pod v1.1
            ServiceLB->>NewPods: 
            
            Note over K8s: Repite proceso...
            K8s->>NewPods: Crea otro Pod v1.1
            K8s->>OldPods: Termina otro Pod v1.0
            
            Note over K8s: ...hasta que todos los Pods son v1.1
            K8s->>NewPods: Finalmente: 3 Pods v1.1 activos
            ServiceLB->>NewPods: Tráfico dirigido solo a v1.1
        ```
  ...
  * **2. `Blue/Green Deployment` (El Clon Seguro):**
    ...
    1.  Despliegas la nueva versión en el entorno Green y realizas pruebas.
    2.  Cuando Green está verificado, cambias el `router` (ej. `K8s Service selector`, `DNS`, `load balancer`) para que todo el tráfico apunte a Green. Green se convierte en el nuevo Blue.
    3.  El entorno Blue original se mantiene por un tiempo como `backup` para un `rollback` instantáneo.
    ...
    <!-- end list -->
      ...
      * **Diagrama `Mermaid` de `Blue/Green`:**
        ```mermaid
          graph TD
            LB["Load Balancer o Router"] --> BlueEnv["Blue Environment: App v1.0 - Activa"]

            subgraph Infraestructura_Duplicada
                BlueEnv --- DB_B["Base de Datos (compartida o dedicada)"]
                GreenEnv["Green Environment: App v1.1 - Pruebas/Inactiva"] --- DB_G["Base de Datos"]
            end

            LB -. "Redirigir tráfico" .-> GreenEnv
            GreenEnv --> EstadoFinal["Estado: Green ahora está Activa"]
            BlueEnv --> Standby["Estado: Blue queda como Hot Standby para Rollback"]

            style BlueEnv fill:#AED6F1,stroke:#3498DB
            style GreenEnv fill:#D5F5E3,stroke:#2ECC71
            style EstadoFinal fill:#D0ECE7,stroke:#27AE60
            style Standby fill:#FADBD8,stroke:#C0392B
  ...
  * **3. `Canary Release` (El Explorador Cauteloso):**
    ...
    1.  Si el canario funciona bien, aumentas gradualmente el porcentaje de tráfico que va a la nueva versión hasta que el 100% del tráfico la esté usando.
    2.  Si el canario muestra problemas, haces `rollback` rápidamente, afectando solo a ese pequeño subconjunto de usuarios.
    ...
    <!-- end list -->
      ...
      * **Diagrama `Mermaid` de `Canary Release`:**
        ...
        graph TD
            ...
            subgraph "Versión Actual (Estable)"
                App_v1_Pool["App v1.0<br/>(95% del Tráfico)"]
            end
            
            subgraph "Nueva Versión (Canary)"
                App_v1_1_Canary["App v1.1<br/>(5% del Tráfico - Canary)"]
            end

            LB -- 95% --> App_v1_Pool
            LB -- 5% --> App_v1_1_Canary

            Monitor[Sistema de Monitorización] --> App_v1_1_Canary
            Monitor -- Feedback --> Decision{¿Canary OK?}
            Decision -- Sí --> LB_Adjust["Aumentar % a v1.1"]
            Decision -- No --> LB_Rollback["Rollback: Todo a v1.0"]

            style App_v1_1_Canary fill:#F9E79F,stroke:#F39C12
        ```
...
Un `rollout` exitoso no garantiza que no haya `bugs` sutiles. Necesitamos formas de revertir rápidamente.
  ...
  * **Basado en Monitorización y Alertas (Prometheus + Alertmanager - 15.8):**
    ...
    <!-- end list -->
      ...
      * **Diagrama `Mermaid`: `Rollback` Automático por Monitorización**
        ```mermaid
          graph TD
            A[Despliegue versión v1.1] --> B[Monitorización con Prometheus]
            B -->|Detecta degradación| C[Alerta crítica enviada por Alertmanager]
            C -->|Webhook o API call| D[Sistema de automatización: scripts o Argo CD]
            D -->|Rollback automático| E[Kubernetes revierte a versión v1.0]
            E --> F[Notificación de rollback exitoso]

            style A fill:#D5F5E3,stroke:#2ECC71
            style B fill:#AED6F1,stroke:#3498DB
            style C fill:#FFCDD2,stroke:#C62828
            style D fill:#FAD7A0,stroke:#F39C12
            style E fill:#E1BEE7,stroke:#8E44AD
            style F fill:#D0ECE7,stroke:#1ABC9C
...
1.  **Docker Official Documentation:**
      * *Enlace:* [docs.docker.com](https://docs.docker.com/) (Especialmente "Get Started", "Develop with Docker", "Dockerfile best practices").
      * *Por qué:* Fundamental para entender `Dockerfiles`, `images`, `containers`, y `Docker Compose`. (Relevante para 15.1, 15.3).
2.  **Semantic Versioning 2.0.0:**
      * *Enlace:* [semver.org](https://semver.org/)
      * *Por qué:* La especificación oficial para el versionado semántico. (Relevante para 15.2).
3.  **Kubernetes Official Documentation:**
      * *Enlace:* [kubernetes.io/docs/](https://kubernetes.io/docs/) (Especialmente "Concepts", "Tasks", "Tutorials").
      * *Por qué:* La fuente definitiva para aprender Kubernetes, sus objetos y cómo operar un `cluster`. (Relevante para 15.4, 15.10).
4.  **Helm Official Documentation:**
      * *Enlace:* [helm.sh/docs/](https://helm.sh/docs/)
      * *Por qué:* Para aprender a empaquetar y gestionar aplicaciones Kubernetes con `Charts`. (Relevante para 15.4).
5.  **Kustomize Official Documentation:**
      * *Enlace:* [kustomize.io/](https://kustomize.io/) (y la documentación de `kubectl apply -k`).
      * *Por qué:* Para la personalización declarativa de `manifests` Kubernetes. (Relevante para 15.4).
6.  **GitHub Actions Documentation:**
      * *Enlace:* [docs.github.com/en/actions](https://docs.github.com/en/actions)
      * *Por qué:* Para construir `pipelines` CI/CD nativos en GitHub. (Relevante para 15.5).
7.  **Argo CD Official Documentation:**
      * *Enlace:* [argo-cd.readthedocs.io](https://argo-cd.readthedocs.io/)
      * *Por qué:* Guía completa para implementar GitOps `Continuous Delivery` con Argo CD. (Relevante para 15.6).
8.  **`structlog` Official Documentation:**
      * *Enlace:* [www.structlog.org/en/stable/](https://www.structlog.org/en/stable/)
      * *Por qué:* Para dominar el `logging` estructurado en Python. (Relevante para 15.7).
9.  **Prometheus Official Documentation:**
      * *Enlace:* [prometheus.io/docs/](https://prometheus.io/docs/introduction/overview/)
      * *Por qué:* Para aprender sobre monitorización con Prometheus, PromQL, y `exporters`. (Relevante para 15.8).
10. **Grafana Official Documentation:**
      * *Enlace:* [grafana.com/docs/grafana/latest/](https://grafana.com/docs/grafana/latest/)
      * *Por qué:* Para crear `dashboards` y visualizar tus métricas (y `logs` si usas Loki). (Relevante para 15.8, 15.9).
11. **Elastic Stack (ELK) Documentation:**
      * *Enlace:* [www.elastic.co/guide/index.html](https://www.elastic.co/guide/index.html)
      * *Por qué:* Para `logging` centralizado a gran escala con Elasticsearch, Logstash, y Kibana. (Relevante para 15.9).
